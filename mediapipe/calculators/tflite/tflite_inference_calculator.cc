...
// Calculator Core Section
    ...
    ::mediapipe::Status TfLiteInferenceCalculator::Process(CalculatorContext* cc) {
      ...
      if (gpu_input_) {
//          LOG(INFO) << "After read 1 " << model_path_<< std::endl;
...
//        LOG(INFO) << "After read 2 " << model_path_<< std::endl;
#elif defined(MEDIAPIPE_IOS)
        const auto& input_tensors =
        cc->Inputs().Tag("TENSORS_GPU").Get<std::vector<GpuTensor>>();
    RET_CHECK_GT(input_tensors.size(), 0);
...
//        LOG(INFO) << "At cpu read" << std::endl;
      ...
      // 2. Run inference.
      if (gpu_inference_) {
//        LOG(INFO) << "At inference" << std::endl;
      ...
      if (gpu_output_) {
        ...
        LOG(INFO) << "At post process"  << model_path_ << std::endl;
    ...
    ::mediapipe::Status TfLiteInferenceCalculator::Close(CalculatorContext* cc) {
      if (delegate_) {
        if (gpu_inference_) {
          ...
          LOG(INFO) << "At close" << model_path_ << std::endl;
...
//      LOG(INFO) << "Loaded the model " << model_path_ << std::endl;
    ...
    ::mediapipe::Status TfLiteInferenceCalculator::LoadDelegate(
            CalculatorContext* cc) {
//        LOG(INFO) << "Delegated called " << model_path_ << std::endl;
#if defined(MEDIAPIPE_ANDROID)
      if (!gpu_inference_) {
...
//              LOG(INFO) << "Tensors " << i << " - " <<  d << " -  " <<tensor->dims->data[d]  << std::endl;
